...
public interface DiyMaterialService extends TenantSafeService<DiyMaterial> {
     ...
     */
    IPage<DiyMaterial> pageMine(int pageNum, int pageSize, Long groupId, boolean ungrouped, String keyword, String type, boolean recycled);
    ...
    /** 移入回收站。物理文件保留——已发布页面可能仍在引用该 URL。 */
    void remove(Long id);

    void restore(Long id);

    /** 永久移除文件库记录，仍保留物理文件，避免破坏已发布内容。 */
    void permanentlyRemove(Long id);
}
