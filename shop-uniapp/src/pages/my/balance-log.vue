...
import { ref } from 'vue'
import { getMyBalanceLogs } from '@/api/index'
...
async function load() {
  loading.value = true
  try {
    const page = await getMyBalanceLogs({ pageNum: pageNum.value, pageSize })
...
function sceneText(s) {
  return ({ recharge: '充值', consume: '消费', refund: '退款', admin: '后台调整', commission: '佣金' })[s] || s || '—'
}
function money(v) {
  const n = Number(v || 0)
  ...
  <view class="page">
    <view class="list">
      <view v-for="item in rows" :key="item.id" class="row">
        ...
        <view class="money" :class="Number(item.money || 0) >= 0 ? 'pos' : 'neg'">{{ money(item.money) }}</view>
      </view>
      <view v-if="!loading && rows.length === 0" class="empty">暂无余额明细</view>
